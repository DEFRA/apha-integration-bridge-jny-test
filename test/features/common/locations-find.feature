...
Feature: Locations endpoint tests - find locations in batch
  ...
  Scenario: 11 Verify PII-authorised client receives unmasked location PII
    ...
    When the request is processed by the system
    Then the locations find API should return unmasked PII fields

  Scenario: 12 Verify that an explicit page and pageSize return only ids from that page slice
  ...
  Scenario Outline: 13 Verify pagination continues when missing ids appear before later valid ids
