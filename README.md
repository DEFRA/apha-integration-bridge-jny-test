...
- `test/features/common/case.feature` and `test/features/common/users-find-by-email.feature` are excluded by default in all environments because the backing case-management routes are not consistently available. Set `CASE_MANAGEMENT_ENABLED=true` to include them again.
...
- `CASE_MANAGEMENT_ENABLED=true` to opt back into `case.feature` and `users-find-by-email.feature`.
